...
# Inisialisasi pipeline Whisper Bahasa Indonesia
stt_pipe = pipeline(
...
@app.post("/stt_streaming/")
async def stt_streaming(file: UploadFile = File(...)):
    """
    Streaming Speech-to-Text endpoint menggunakan model Whisper Bahasa Indonesia.
    """
    try:
        # Baca file audio ke memori
        audio_bytes = await file.read()
        audio_buffer = io.BytesIO(audio_bytes)
        # Jalankan transkripsi
        # NOTE: pipeline whisper tidak native streaming, jadi kita kirim hasil sekaligus
        result = stt_pipe(audio_buffer)
        text = result["text"] if isinstance(result, dict) else result
        # Kembalikan hasil sebagai streaming response (bisa diubah ke chunk jika ingin split sentence)
        def text_stream():
            # Untuk demo, kirim per kalimat
            for sentence in re.split(r'(?<=[.!?])\s+', text):
                if sentence.strip():
                    yield sentence.strip() + "\n"
        return StreamingResponse(text_stream(), media_type="text/plain")
    except Exception as e:
        logging.error(f"Error in stt_streaming: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/ollama_chat/")
async def ollama_chat(prompt: str = Form(...), model: str = Form("llama3")):
    """
    Endpoint untuk generate AI chat menggunakan Ollama local.
    Kirim pertanyaan (prompt) dan dapatkan jawaban dari model Ollama.
    """
    try:
        # Kirim permintaan ke Ollama REST API
        ...
        logging.error(f"Error in ollama_chat: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
